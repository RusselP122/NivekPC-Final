...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["productImage"]["tmp_name"], $targetFile)) {
            ...
            $sql = "INSERT INTO new_arrivals (product_name, product_price, stock, description, product_img) VALUES (?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            if ($stmt->execute([$productName, $productPrice, $productStock, $productDescription, basename($_FILES["productImage"]["name"])])) {
                // Redirect to the appropriate page
                header("Location: manage_newarrivalproduct.php");
                exit;
            } else {
