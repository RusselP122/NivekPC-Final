    ...
    try {
        ...
        if(isset($_FILES['productImage']) && $_FILES['productImage']['error'] === UPLOAD_ERR_OK) {
            ...
            if(move_uploaded_file($_FILES['productImage']['tmp_name'], $uploadDirectory . $fileName)) {
                ...
                $stmt = $pdo->prepare("UPDATE new_arrivals SET product_name = :productName, product_price = :productPrice, stock = :productStock, description = :productDescription, product_img = :productImage WHERE product_id = :productId");
                ...
                header("Location: manage_newarrivalproduct.php?status=file_error");
                exit();
            }
        } else {
            // If no new file was uploaded, update the database record without changing the image
            $stmt = $pdo->prepare("UPDATE new_arrivals SET product_name = :productName, product_price = :productPrice, stock = :productStock, description = :productDescription WHERE product_id = :productId");
        ...
        header("Location: manage_newarrivalproduct.php?status=success");
        exit();
    } catch (PDOException $e) {
        // Redirect back to the page with an error message if the update fails
        header("Location: manage_newarrivalproduct.php?status=error&message=" . urlencode($e->getMessage()));
        exit();
    }
}
?>
